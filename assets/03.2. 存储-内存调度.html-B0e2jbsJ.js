import{_ as n,c as i,b as r,a as e,d as t,o as s}from"./app-CazEcPAh.js";const a="/assets/%E6%83%B0%E6%80%A7%E5%88%A0%E9%99%A4-L0VraUoV.jpg",o="/assets/%E5%AE%9A%E6%97%B6%E5%88%A0%E9%99%A4%E6%B5%81%E7%A8%8B-rOfd_EJy.jpg",d="/assets/lru%E5%AD%97%E6%AE%B5-owmoRW_R.png",p={};function _(u,l){return s(),i("div",null,[...l[0]||(l[0]=[r('<h1 id="redis-内存" tabindex="-1">Redis 内存</h1><h2 id="redis-过期删除" tabindex="-1">Redis 过期删除</h2><h3 id="过期时间的设置和存储-过期字典、键值删除策略" tabindex="-1">过期时间的设置和存储 (过期字典、键值删除策略)</h3><p>Redis 是可以对 key 设置过期时间的，因此需要有相应的机制将已过期的键值对删除，而做这个工作的就是<strong>过期键值删除策略</strong>。</p><p>每当我们对一个 key 设置了过期时间时，Redis 会把该 key 带上过期时间存储到一个<strong>过期字典</strong>（expires dict）中，也就是说「过期字典」保存了数据库中所有 key 的过期时间。</p><h3 id="过期删除策略" tabindex="-1">过期删除策略</h3><p>Redis 使用的过期删除策略是「<strong>惰性删除+定期删除</strong>」这两种策略配和使用。</p><p>惰性删除策略和定期删除策略都有各自的优点，所以 Redis 选择「惰性删除+定期删除」这两种策略配和使用，以求在合理使用 CPU 时间和避免内存浪费之间取得平衡。</p><h4 id="惰性删除策略" tabindex="-1">惰性删除策略</h4><p>惰性删除策略的做法是：<strong>不主动删除过期键，每次从数据库访问 key 时，都检测 key 是否过期，如果过期则删除该 key。</strong></p><p>当我们查询一个 key 时，Redis 首先检查该 key 是否存在于过期字典中：</p><ul><li>如果不在，则正常读取键值；</li><li>如果存在，则会获取该 key 的过期时间，然后与当前系统时间进行比对，如果比系统时间大，那就没有过期，否则判定该 key 已过期。</li></ul><p>惰性删除的流程图如下：</p><p><img src="'+a+'" alt="img" loading="lazy"></p><ul><li><p>优点</p><ul><li><p>损耗少、性能好，CPU友好。</p><p>因为每次访问时，才会检查 key 是否过期，所以此策略只会使用很少的系统资源，因此，惰性删除策略对 CPU 时间最友好。</p></li></ul></li><li><p>缺点</p><ul><li><p>内存浪费，内存不友好。</p><p>如果一个 key 已经过期，而这个 key 又仍然保留在数据库中，那么只要这个过期 key 一直没有被访问，它所占用的内存就不会释放，造成了一定的内存空间浪费。所以，惰性删除策略对内存不友好。</p></li></ul></li></ul><h4 id="定期删除" tabindex="-1">定期删除</h4><p>定期删除策略的做法是，<strong>每隔一段时间「随机」从数据库中取出一定数量的 key 进行检查，并删除其中的过期key。</strong></p><p>Redis 的定期删除的流程：</p><ol><li>从过期字典中随机抽取 20 个 key；</li><li>检查这 20 个 key 是否过期，并删除已过期的 key；</li><li>判断本轮检查的已过期 key 的数量 <ul><li>如果超过 5 个（20/4），也就是「已过期 key 的数量」占比「随机抽取 key 的数量」大于 25%，则继续重复步骤 1</li><li>如果已过期的 key 比例小于 25%，则停止继续删除过期 key，然后等待下一轮再检查</li></ul></li></ol><p>可以看到，定期删除是一个循环的流程。那 Redis 为了保证定期删除不会出现循环过度，导致线程卡死现象，为此增加了定期删除循环流程的时间上限，默认不会超过 25ms。</p><p>定期删除的流程如下：</p><p><img src="'+o+'" alt="img" loading="lazy"></p><ul><li><p>优点</p><ul><li>通过<em>限制</em>删除操作执行的<em>时长和频率</em>，来减少删除操作对 CPU 的影响，同时也能删除一部分过期的数据减少了过期键对空间的无效占用。</li></ul></li><li><p>缺点</p><ul><li><p>难以确定删除操作执行的时长和频率。</p><p>如果执行的太频繁，就会对 CPU 不友好；</p><p>如果执行的太少，那又和惰性删除一样了，过期 key 占用的内存不会及时得到释放。</p></li></ul></li></ul><p>TIP</p><blockquote><p>Redis 的过期删除的内容就暂时提这些，想更详细了解的，可以详细看这篇：<a href="https://xiaolincoding.com/redis/module/strategy.html" target="_blank" rel="noopener noreferrer">Redis 过期删除策略和内存淘汰策略有什么区别？</a></p></blockquote><h3 id="持久化时-如何处理过期键" tabindex="-1">持久化时，如何处理过期键？</h3><p>Redis 持久化文件有两种格式：RDB（Redis Database）和 AOF（Append Only File），下面我们分别来看过期键在这两种格式中的呈现状态。</p><p>RDB 文件分为两个阶段：</p><ul><li><p><strong>RDB 文件生成阶段</strong>：</p><p>从内存状态持久化成 RDB（文件）的时候，会对 key 进行过期检查，过期键<em>「不会」被保存到新的 RDB 文件中</em>，因此 Redis 中的过期键不会对生成新 RDB 文件产生任何影响。</p></li><li><p><strong>RDB 加载阶段</strong>：</p><p>RDB 加载阶段时，要看服务器是主服务器还是从服务器，分别对应以下两种情况：</p><ul><li>如果 Redis 是「主服务器」运行模式的话，在载入 RDB 文件时，程序会对文件中保存的键进行检查，过期键<em>「不会」被载入到数据库中</em>。所以过期键不会对载入 RDB 文件的主服务器造成影响；</li><li>如果 Redis 是「从服务器」运行模式的话，在载入 RDB 文件时，<em>不论键是否过期都会被载入到数据库中</em>。但由于主从服务器在进行数据同步时，从服务器的数据会被清空。所以一般来说，过期键对载入 RDB 文件的从服务器也不会造成影响。</li></ul></li></ul><p>AOF 文件分为两个阶段：</p><ul><li><p><strong>AOF 文件写入阶段</strong>：</p><p>当 Redis 以 AOF 模式持久化时，如果数据库某个过期键还没被删除，那么 AOF 文件会<em>保留此过期键</em></p><p>当此过期键被删除后，Redis 会向 AOF 文件<em>追加一条 DEL 命令来显式地删除该键值</em>。</p></li><li><p><strong>AOF 重写阶段</strong>：</p><p>执行 AOF 重写时，会对 Redis 中的键值对进行检查，<em>已过期的键不会被保存到重写后的 AOF 文件中</em>，因此不会对 AOF 重写造成任何影响。</p></li></ul><h3 id="主从模式中-如何处理过期键" tabindex="-1">主从模式中，如何处理过期键？</h3><p>当 Redis 运行在主从模式下时，<strong>从库不会进行过期扫描，从库对过期的处理是被动的</strong>。也就是即使从库中的 key 过期了，如果有客户端访问从库时，依然可以得到 key 对应的值，像未过期的键值对一样返回。</p><p>从库的过期键处理依靠主服务器控制，<strong>主库在 key 到期时，会在 AOF 文件里增加一条 del 指令，同步到所有的从库</strong>，从库通过执行这条 del 指令来删除过期的 key。</p><h2 id="redis-内存淘汰" tabindex="-1">Redis 内存淘汰</h2><p>什么是内存淘汰？ —— (如下)</p><p>内存满了会发生什么？—— 内存淘汰</p><p>在 Redis 的运行内存达到了某个阀值，就会触发<strong>内存淘汰机制</strong>，这个阀值就是我们设置的最大运行内存。</p><p>此值在 Redis 的配置文件中可以找到，配置项为 maxmemory。</p><p>TIP</p><blockquote><p>Redis 的内存淘汰的内容，想更详细了解的，可以详细看这篇：<a href="https://xiaolincoding.com/redis/module/strategy.html" target="_blank" rel="noopener noreferrer">Redis 过期删除策略和内存淘汰策略有什么区别？</a></p></blockquote><h3 id="八种内存淘汰策略" tabindex="-1">八种内存淘汰策略</h3><p>Redis 内存淘汰策略共有八种</p><p>这些策略可以按淘汰范围分类：</p>',44),e("div",{class:"ab-note drop-shadow"},[e("div",{class:"ab-list-table-parent"},[e("table",{class:"ab-table ab-list-table"},[e("tbody",null,[e("tr",{class:"ab-foldable-tr",tr_level:"0",is_fold:"false",able_fold:"true",type:"不进行数据淘汰的策略"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
          ...
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"不进行数据淘汰的策略")])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"1",is_fold:"false",able_fold:"false",type:"**noeviction**"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            const l_tr = tr.parentNode.querySelectorAll("tr")
            const i = 1
          ...
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("strong",null,"noeviction")])])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("em",null,"不淘汰")])])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"它表示当运行内存超过最大设置内存时，不淘汰任何数据，而是不再提供服务，直接返回错误")])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("em",null,"Redis3.0 之后"),t("，默认的内存淘汰策略")])])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"0",is_fold:"false",able_fold:"true",type:"进行数据淘汰的策略"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            ...
            if (flag_do_fold) tr.setAttribute("is_fold", tr_isfold=="true"?"false":"true")
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"进行数据淘汰的策略")])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"1",is_fold:"false",able_fold:"true",type:"在设置了过期时间的数据中进行淘汰"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            const l_tr = tr.parentNode.querySelectorAll("tr")
            const i = 3
            ...
            if (flag_do_fold) tr.setAttribute("is_fold", tr_isfold=="true"?"false":"true")
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"在设置了过期时间的数据中进行淘汰")])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"2",is_fold:"false",able_fold:"false",type:"**volatile-TTL**"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            const l_tr = tr.parentNode.querySelectorAll("tr")
            const i = 4
          ...
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("strong",null,"volatile-LFU")])])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("em",null,"Least Frequently Used，最近最不常用")])])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"淘汰所有设置了过期时间的键值中，最少使用的键值")])])]),e("td",{rowspan:"1"},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,[e("em",null,"Redis4.0 之后"),t("，新增的内存淘汰策略")])])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"1",is_fold:"false",able_fold:"true",type:"在所有数据范围内进行淘汰"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            ...
            if (flag_do_fold) tr.setAttribute("is_fold", tr_isfold=="true"?"false":"true")
          `},[e("div",{class:"ab-list-table-witharrow markdown-rendered"},[e("div",null,[e("p",null,"在所有数据范围内进行淘汰")])])])]),e("tr",{class:"ab-foldable-tr",tr_level:"2",is_fold:"false",able_fold:"false",type:"**allkeys-Random**"},[e("td",{rowspan:"1",onclick:`            const tr = (this.tagName == "TD") ? this.parentNode : this
            const l_tr = tr.parentNode.querySelectorAll("tr")
            const i = 9
